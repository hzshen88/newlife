...
- **日期**：2026-09-01　**状态**：v0.10（v0.1a/v0.1b 均已判定：H1 支持，`compatible_for_frozen_slices_v1_lowered` 与 `staging_declarable_for_frozen_slices`；**v0.2 gate 已关闭**：World 1 = Resource Foraging 双条件 L2 逐位精确复现，`results/v0.2/gate.json` `passed: true`；**v0.3 compare 主线已判定**：H1 支持，`causal_attribution_declarable_for_world1_pairs`，预注册冻结于 exloop `322a8dc8`，实现见 `packages/newlife/src/newlife/core/compare.py`；**第二世界（`ms` 最小溯祖模型）已判定**：H1 支持，`ms_minimal_coalescent_reproducible`，预注册冻结于 exloop `c450072`，实现见 `packages/newlife/src/newlife/mechanisms/second_world/`；**第四世界（Moran 谱系 vs Kingman 溯祖）已判定**：H1 支持，预注册冻结于 exloop `24bb02e`，且 goal 的痛点 P1 首次判为 `achieved`——**移离零，不是解决**，见 [`docs/zh/worlds/004-moran-genealogy.md`](../worlds/004-moran-genealogy.md)；**第五世界（gate 自身的检查覆盖率）已判定**：**H0 支持**——47 个 check 里 3 个 `hollow`，八个里程碑以来第一个非 H1 结果，见 [`docs/zh/worlds/005-gate-check-coverage.md`](../worlds/005-gate-check-coverage.md)；**第十九个里程碑（产率是输入还是结果）已判定**：**H1**——两个第三方积木共用同一条摄取律，差别只在摄取之后；扫描 `EX_o2_e` 得教科书式溢流代谢，**唯象模型把产率当输入，机制模型把产率当结果**。**其负控 Z5 事后查出恒真、判定力为零**（错在冻结的预注册，非实现）；**H1 保留、产物不覆盖**，结论由 Z3∧Z4 的算术独立支撑。**并查出六个门搬进 newlife 后 `run_gates.py` 八个门全部打不开文件——单门自检全绿而日常聚合入口是死的**，见 [`docs/zh/worlds/019-yield-input-or-outcome.md`](../worlds/019-yield-input-or-outcome.md) §6；**第十八个里程碑（推导权限声明）已判定**：**H0**——判据要求逐项相同，`claims` 只有 1/3，故按字面未达成；**但「推不出来」的项是零**：`allowed_effects` 3/3、**`add`/`set` 算符 3/3**（**文献点名「从接口类型推不出来」的正是这一项**，用**行为探针**问 store 类型「1.0 收到 0.5 会变成什么」推出来），两处 `claims` 差异经**机械分类**都是手写声明当初的选择或缺陷（`layout` / `finer`），不是第三方没交付信息。**瓶颈是可以拆掉的**，见 [`docs/zh/worlds/018-derive-authority.md`](../worlds/018-derive-authority.md)；**第十七个里程碑（求解器背后的第三方 process）已判定**：**H1**——`spatio_flux.processes.dfba.DynamicFBA`（每 tick 解一个 LP，E. coli core）源码零改动接入，轨迹与**裸 pb**逐字节相同，负控被拒**且异常来自 newlife 契约层**，元负控成立。**H1 之下必须连报三件事**：① **逐字节复现在这里买到的是「同一个求解器的同一个选择」，不是「同一个科学答案」**（FBA 最优解通常不唯一），所以「记录求解器身份」被列进**合取**；② **GLPK 是第三条非 Python 依赖**，第十六个里程碑顺带带进来、无人提醒——**穷举有有效期**，且 PATH 屏蔽实验够不着共享库；③ **端口的粒度限制了声明的粒度**。见 [`docs/zh/worlds/017-solver-backed-process.md`](../worlds/017-solver-backed-process.md)；**第十六个里程碑（接真正独立的第三方包）已判定**：**H1**——`spatio_flux.processes.monod_kinetics.MonodKinetics`（**`spatio-flux==1.4.0`，独立发行包**）源码零改动接入，与**裸 pb**的 `mass`/`exchange` 两条轨迹逐字节相同，两条负控被拒且状态不变，**元负控成立**。**第十五个里程碑「生态不通」的结论收回一半**——不通的是那两个入口包，不是整个生态。**撞出一处真发现**：`lowering_table` 按 Effect 类别做键，**一个机制每类 Effect 只能写一个端口**，单端口的 `Grow` 看不出、两个端口就塌（第九世界「少于三个 provider 不许定型」的又一次）。成色仍有保留：**词表来自第三方（`register_types`），权限仍由我们代写**，见 [`docs/zh/worlds/016-external-package.md`](../worlds/016-external-package.md)；**第十五个里程碑（接第三方 process）已判定**：**H1**——`process_bigraph.processes.growth_division.Grow` **源码零改动**被接进一个 newlife 世界，轨迹与**裸 pb**逐字节相同，两条负控（声明错路径 / 未声明 Effect）都被拒且状态不变，**元负控成立**（绕开契约就不被拒）。**但「接上 pb 就接上了它的生态」今天不成立**：`biosimulator-processes` 0.3.19 与 `vivarium-interface` 0.0.5 **都装不起来**（都 import 一个任何可用 pb 版本里都不存在的 `ProcessTypes`，且都没声明版本下界）。**成色须降级**：端口→(路径,算符) 那张表只能由我们代写，对照 FMI（接口描述由模型作者随实现交付）——H1 证明的是「契约对**被代写的声明**有强制力」，见 [`docs/zh/worlds/015-foreign-process.md`](../worlds/015-foreign-process.md)；**第十四个里程碑（跨运行时 verification）已判定**：**H1**——World 4 的同一份声明在 `ReferenceKernel` 与 `process-bigraph` 上各跑 2101 次，**2101/2101 逐字节相同**，负控（改坏 pb 的 store 写入）变红，6.8s；**process-bigraph 从零使用变成真跑通一个世界**。**但 goal 只买到一半**：pb **不满足** B 接缝的 Definition——`core/runtime.py` 的四个操作是从 RK 读出来的逐阶段 **pull** 形状，而 pb 是先拿整张图再自己调度的 **push** 运行时，**B 今天仍只有一个 provider**；此事「事前未预见」。顺带闭合了「契约层焊死在一个运行时上」，并查出**第七世界的判定记录自第八个里程碑起是烂的**（重跑得 INVALID 而非记录的 H0），见 [`docs/zh/worlds/014-cross-runtime-verification.md`](../worlds/014-cross-runtime-verification.md)；**第十三个里程碑（非 Python 依赖）已判定**：**H0**——`git` 缺失时静默产出一份格式完好的判定，理由写成「基线不可得」而真实原因是 git 不在，**第四次栽在同一形状且环境层扫描器够不着**，见 [`docs/zh/worlds/013-undeclared-deps.md`](../worlds/013-undeclared-deps.md)；**第十二个里程碑（归档重放）已判定**：H1，5/5，但测出未声明的 C 工具链依赖，且**无独立预注册**；**第十一个里程碑（旧判定复现率）已判定**：H1 支持——除已知的 seventh 外**七个全部逐字节复现**；口径须连报：文献 3.4% 的基准率来自跨年依赖漂移，本项目是同环境几天之内，**跨年漂移从未被测过**，见 [`docs/zh/worlds/011-verdict-rot.md`](../worlds/011-verdict-rot.md)；**第十个里程碑（判定接缝）已判定**：**INVALID**（IC-4）——三代判定表示已装进同一个三值 Definition 且值域未放宽，但 `seventh` 的冻结产物因第八世界改动而不可复现；**顺带发现第七世界的判定已失效**，见 [`docs/zh/worlds/010-verdict-seam.md`](../worlds/010-verdict-seam.md)；**第九世界（tick 循环终止条件）已判定**：**H0-a 支持**——终止条件能用冻结算符集表达（C2 通过、未触发 inner-platform），但 `run` 是混合部件且 harness 与 World 1 装配所有权冲突；**卡住的是部件边界与装配所有权，不是表达能力**，见 [`docs/zh/worlds/009-harness-tick-loop.md`](../worlds/009-harness-tick-loop.md)；**第八世界（harness 吃下 World 4）已判定**：H1 支持——**P3 首次成立**（受限：World 4 无 tick 循环，`simulator` 块最难的一半未检验）；`world.py` 148 → 68 行，配置纯数据、verdict 逐位不变，见 [`docs/zh/worlds/008-harness-absorbs-world4.md`](../worlds/008-harness-absorbs-world4.md)；**第七世界（harness 可生成性）已判定**：**H0 支持**——42 个部件里 4 个落不进 DEVS 三分+rng，聚成「数值复现」与「运行时契约强制」两种能力，后者正是 P2；通用 harness 的规格是五块不是三块。**其判定产物自第八个里程碑（`6bbc05c` 删掉 `MoranGenealogyWorld`）起状态为 `unevaluable`**——重跑触发「声明幽灵部件 → INVALID」，那正是原判定自己测过的负控。**处置：H0 保留，不改声明、不覆盖产物**（IC-3 只管「判定前」的改动；4 个 `unclassified` 全在第八个里程碑没碰的 `resource_foraging/world.py`；稳健性探测与旧 runner 对照都确认结论不变）。见 [`docs/zh/worlds/007-harness-generability.md`](../worlds/007-harness-generability.md) §6；**第六世界（staging 绕过可归约性）已判定**：H1 支持——四个调度循环全部 unrolling 可表达，「五次同因」裁定为不成立（三种 bound 类型），`staging.py` 无需为「重复到吸收」扩展，见 [`docs/zh/worlds/006-staging-bypass-reducibility.md`](../worlds/006-staging-bypass-reducibility.md)；**第三世界（Moran 选择可判定性）已判定**：H1 支持，预注册冻结于 exloop `aa052dd`，证据分层裁定落地——claim (i) 为与 L1/L2/L3 正交的 analytic/specification-conformance，见 [`docs/zh/worlds/003-moran-selection.md`](../worlds/003-moran-selection.md)）
...
> **（2026-09-04 更正）表中所有关于 `prereg.sh audit` 通过的记载，只覆盖
> FROZEN / STAMP / INTEGRITY 三项，不覆盖 CHRONOLOGY。** 预注册在 `exloop`、判定产物在
> `newlife`，两个仓库**没有共同的 git 祖先**，chronology 检查因此**一次都没有真正执行过**
> ——旧版脚本在检查了零个产物之后仍然打印 `predictions pre-date outputs`，而传入 newlife
> 的绝对路径时 git 明确报 `outside repository`、脚本照样报通过。
> **22 份预注册用修好的脚本重跑，全部报 PARTIAL：chronology 从未建立。**
>
> **顺序本身仍然可证，只是不由那个脚本证**：15 份产物在自己的 JSON 里记录了冻结提交的
> SHA（如 `results/nineteenth/summary.json` 的 `"preregistration": "0e04160"`），而
> **一个 SHA 在它的提交存在之前无法被写出来——指名即晚于**。早期几份
> （v0.1a/b、v0.2、v0.3、second-world、fifth-world）**没有记录，它们没有任何机械顺序证据**。
>
> 根因是「预注册与产物分居两个仓库」。这正是产品化要解决的事：
> **一个问题一个仓库，预注册与产物同仓，chronology 才有意义。**
>
> **（2026-09-04 更新，已落地）措辞收窄**：证据只逼出「同仓」，没逼出「一个问题一个仓库」——
> 后者意味着 N 次 `git init`、N 个远端、无法共享辅助代码。**实测确认：一个仓库多个问题
> 文件夹同样成立**（q2 的产物早于 q1 的冻结，按 `--results` 划范围后 q1 仍 PASS）。
> 已实现为 `newlife init / freeze / run / check / audit`，`prereg.sh` 随包发布。
> **`check` 的四条门打在用户自己的文件上**：goal 就绪 · 假扫描 · 静默退化 ·
> 预注册↔runner 单元对齐（第三条是新建的——第一次真实使用就抓到一处：预注册写
> `S0∧S1∧S2∧S3` 而 runner 只算三个，**合取比它看起来的弱一格**）。
> **（2026-09-05 更新）goal 就绪门已随包发布**：`newlife init` 生成一份**故意是红的**
> `goal.md`，`newlife freeze` 在冻结前挡一道。此前 skill 会让 AI 写出六个锚而验锚的门
> 不发布——实测四份真实 `prereg.md` 锚数是 0·0·0·0。**阻塞点在 freeze 不在 check**：
> `check` 里的单元对齐要读 `results/`，跑不动在冻结前。
> 其余两条门仍不随包发布：它们认的是验证脚本台账与 goal/question/ledger 三元组，
> **假定了另一套文档流水线**——这是归属，不是省略。
> **在全新安装的环境里从零走完一遍，四项检查全 PASS，chronology 首次真正建立。**

| 里程碑 | 内容 | 验收 |
|---|---|---|
| **v0.1a lowering 完备性**（✅ 预注册实验已判定 2026-08-30；v0.1 拆分后唯一重预注册项） | 预注册问题："删除 engine_update 后，由 lowering 从 Effect 机械生成的 update，能否在四个切片上复现冻结 trace 逐字节一致"——判据**直接复用**压力测试冻结的 expected JSON 与规范 trace，不写新 expected。架构切两半以便失败可诊断：core 定义中间表示 `(path, op, payload, provenance)`，op ∈ `{set, add, transfer_pair, structural, contribution_resolve, event}`（R2，六值）；adapter 只做 IR → runtime 形状的逐 store 类型翻译。失败时定位为"Effect → IR 缺信息（契约问题，走 §5.6）"或"IR → update 翻译错（adapter bug）"。**H0（冻结版，经修正）**：至少一个冻结 cell 的 update 不是四元基 (Effect 载荷, 注册 schema 类型, 机制身份, 观测态视图) 的纯函数——hidden instance state、wall clock、live RNG 明确在基外。**修正记录**：起草期曾立"StateDelta 载荷不区分 set/add"为具名否定点，源码核查证伪（契约 v1 的 `StateDelta.operation ∈ {add, set}` 一直存在），plan 起草时替换为上述四元基 H0，冻结于 prereg `62d6d91`。同时完成：⓪ 移植信任核心 proofroot（只搬 D1 现行编码 + D4 不变量 + D2/D5，legacy 兼容层一律不搬；EvidenceCore.jl 冻结为 oracle 生成跨语言向量，§5.10）；封闭 engine_update 旁路、serialization spec 落地（§5.9）、重推导 conform 契约级套件、AST/import-lint CI | **判定（2026-08-30）：H1 支持，`compatible_for_frozen_slices_v1_lowered`**——冻结 trace 逐字节复现 ✓；重推导契约级套件全绿（94 项）✓；import-lint 通过（R4.2 引擎标识符禁令入 CI 门）✓；旁路封闭以新增负例证明 ✓；audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注）。证据：newlife `results/v0.1a/`（活）与 exloop `artifacts/lowering-results/`（档案） |
| **v0.1b staging 可声明化**（✅ 预注册实验已判定 2026-08-31；依赖 0.1a） | 声明形状最小化（评审裁定，2026-08-31）：staging schema 置于既有自由 `schedule` 字段（不加字段——`MechanismSpec` dataclass 形状属契约 v1）；profile 校验 DAG 无环；`staging.py` 按拓扑序生成 Composite 编排。判据 = 复现压力测试 harness 手工连线产出的冻结 trace。单 Composite 语义保持开放（风险 3）——0.1b 只证明"多 Composite 编排可从声明推导"，已足以把调度语义从 harness 代码搬进声明 | **判定（2026-08-31）：H1 支持，`staging_declarable_for_frozen_slices`**（v0.1a 判定同时保持）——冻结 trace 复现 ✓（P1–P4 程序化断言：编译器记录 == 冻结预测）；DAG 无环等 8 项具名负例通过 ✓（含编排篡改对照被字节比对咬住、手工 Composite 构造 lint 零豁免）；参考内核程序序重复感知谓词校验 ✓；audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注）。证据：newlife `results/v0.1b/`（活）与 exloop `artifacts/staging-results/`（档案）；staging schema 置于 `schedule` 字段（评审裁定），profile 2.1.0 承载 schema v1，conform 套件 1.1.0 |
...
| **v0.2 机制箱（已关闭）** | **Gate 已通过（2026-08-31）：第一个世界问题 = Parworlds Experiment 001 Resource Foraging（`resource-foraging-v1`）**——选择理由、冻结已知答案、三层比对方法（L1 派生种子已逐位验证、L2 录制 draw 注入优先、L3 统计比较退路）、宪法→stage/机制注册表的映射与痛点对照框架，见 [`docs/zh/worlds/001-resource-foraging.md`](../worlds/001-resource-foraging.md)。000（Memory Benchmark）落选理由同文档。这个选择一次解决四件事：① 真问题——不是为库定制的表演性问题；② 已知答案——冻结的 Julia 结果即 known answer；③ 跨语言双实现比对——Julia 参考 vs Python newlife，压力测试方法学的放大版，serialization spec 第一天就有真实载荷；④ compare 归因判据的校准来源——该世界当年做过/想做的消融。**比对方法（种子可移植、序列不可移植）**：跨语言一致性验到派生种子层为止（§5.10 合同边界）；Julia 的 Xoshiro 流与 Python 生成器不同，"已知答案"**不靠活 RNG 的字节级 trace 比对**。两个可行方法，优先前者：① 录制 Julia 冻结 run 的 draw 序列注入 newlife——这正是压力测试已验证的注入方法，冻结 run binding 保证重跑可提取；② 退到观测量层的统计比较。机制注册表条目全部由该问题导出；四个人造切片不作为生产机制迁入 | **验收（2026-08-31）**：世界问题文档化 ✓；双条件（informative/cue-neutral）全量 32×32、5000-tick、含 128-episode assay 的 L2 录制 draw 注入比对逐位精确复现（`verdict: reproduced`，全部命名流耗尽为 0）✓；注册表条目由该问题导出（6 机制）✓；示例应用零 runtime 代码（import-lint 证明）✓；**痛点对照**：P3 世界重表达/harness 行数对照如实记录（newlife 两侧行数均高于 Julia，收益在结构不在行数）。证据：newlife `results/v0.2/gate.json`（`passed: true`），详见 `docs/zh/worlds/001-resource-foraging.md` |
| **v0.3 compare 主线（已判定）与第二世界（已判定，`ms` 最小溯祖模型）** | **compare 半判定（2026-08-31）：H1 支持，`causal_attribution_declarable_for_world1_pairs`**——question 文档起草时发现的真实缺口（informative/cue-neutral 的唯一受控差异 `condition` 完全不出现在 `MechanismSpec` 声明里，registry diff 会得到空集）经两轮红队修正后，归因算法改为"域无关字段 diff + 观测序列分歧定位 + 反事实单字段翻转重跑"，聚合终态按固定优先级判 `causal`/`incidental`/`UNATTRIBUTED_DIVERGENCE`/`MULTI_CAUSAL_OR_UNRESOLVED`。预注册（audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））冻结于 exloop `322a8dc8`。7 个标定单元（4 个 World-1 真实 + 3 个纯合成，9 次执行）全部通过，含专门回归两个终态优先级冲突的 unit 6c。证据：newlife `results/v0.3/summary.json`；实现 `packages/newlife/src/newlife/core/compare.py`（域无关，stdlib-only）+ `mechanisms/resource_foraging/calibration.py`（World-1 校准点）+ `scripts/accept_v03_compare_gate.py`。第二世界最终选型改为 Hudson `ms`（1980 年代经典溯祖模拟器，该领域公认的验证标准答案），**不是**最初设想的 SLiM/msprime 对标——question 文档调研发现那条路会把证明标准从"逐位精确"降级为"统计上差不多"（SLiM/msprime 是他人 C++ 代码，无法像 v0.2 那样精确对账），而 `ms` 只有 1851 行、能真正啃透源码，保住了最高标准。**判定（2026-09-01）：H1 支持，`ms_minimal_coalescent_reproducible`**——预注册两轮红队（修正 R2 映射检查空转、`segsites==0` 输出形态未声明、两层判据被单层结果冒领）后冻结于 exloop（`prereg.sh freeze`，全新未提交路径）。两层合取判据：(a) 独立、无依赖的 Python 移植，重放真实、当场编译运行的 `ms` 二进制自身记录的 `drand48()` draw 序列，30 个复制品（3 组种子 × 10 次）segsites 与基因型矩阵逐位精确复现，零剩余零提前耗尽；(b) 同一算法包进 `MechanismSpec`/`StructuralRewrite`/`Event`/`ReferenceKernel` 两机制注册表（`biological` 面溯祖建树逐次合并事件各发一条 `StructuralRewrite`，`evidence` 面观测者读已提交树发真实 `Event`），逐复制品复现 (a) 自身输出。两层全绿，另加 R2 种子映射正确性检查（往返重装 + 高位丢弃可观测）与 `segsites==0` 覆盖率断言。**如实记录：这是第二个连续 bypass `staging.py` 的真实世界（P1 机制可组合性不但不推进，还小幅倒退——最小模型阶段整个溯祖循环刻意收在一个机制里，未拆分，因为拆分一个"多个竞争指数等待里挑最早一个"的事件循环，早于 schedule 形状问题解决之前就是过早结构化，question 文档已如实预告）**。世界文档见 [`docs/zh/worlds/002-ms-coalescent.md`](../worlds/002-ms-coalescent.md)（收尾时漏写，2026-09-01 补记）。证据：newlife `results/second-world/summary.json`（verdict 从合取机械计算，从不手填）+ 30 份逐复制品记录 `results/second-world/replicates/`；实现 `packages/newlife/src/newlife/mechanisms/second_world/`（`ms_coalescent.py` 独立移植 + `mechanisms.py`/`world.py` 注册表）+ `conform/second_world_verdict.py`（verdict runner） | compare：判据见预注册，已核对全绿；第二世界：判据见预注册，两层 30 复制品全绿，`results/second-world/summary.json` `passed: true`；profile 核心（`MechanismSpec`/`Effect` 五类）零改动 |
| **第五世界（已判定）：gate 自身的检查覆盖率** | **判定（2026-09-03）：H0 支持——八个里程碑以来第一个非 H1 结果**。测的是这套审核机制自己的盲区：exloop 四个验证脚本共 47 个 check，逐条归入 `auto`(24) / `hand`(15) / `anchored`(5) / `hollow`(3) / `IC-1`(0)。**三个 `hollow`**：`IC-1-tractable-at-frozen-N` 的判定条件是**字面量 `True`**（任何变异都不可能让它红）；`check-2` 是记账式计数，covers 内的变异改的是值而它数的是个数；`check-5` 是内联字面量算术，不经任何生产函数。**二阶发现**：`covers` 会误归因——它的语义是「自上次 report 起新观测到的调用」，**不等于**「算出这条 check 所断言的那个值的东西」，后两个 `hollow` 由此产生而非真空洞，必须与数字一起报。**实测自动覆盖**：两个 plan 侧脚本 **0/11 与 0/7**；在 `094c6c8` 之前 **18/47 的 check 处于「显示已覆盖、实际从未被测」**（`mutation_scan` 因标签正则写死前缀而在空集上恒真）。**goal 判定 `achieved` 而 verdict 为 H0**——判据是「量出覆盖率并把盲区写下来」，不依赖 H1，§1.5 的两层解耦第一次被真实结果检验；C4（新盲区变 fixture）未做，如实记录。世界文档见 [`docs/zh/worlds/005-gate-check-coverage.md`](../worlds/005-gate-check-coverage.md)。证据：`results/fifth-world/summary.json`（verdict 从合取机械计算，`passed: false`）；runner `conform/fifth_world_verdict.py` | 分类表 47/47 全覆盖、每行有证据、无未讨清 IC-1；`hollow` 计数为 3 故 H0；goal 判定与 verdict 物理分开 |
| **第四世界（已判定）：Moran 谱系 vs Kingman 溯祖，兼 P1 首个实例** | **判定（2026-09-02）：H1 支持**。第一个**没有任何外部程序作为对标**的世界——对标项目自己推导并由脚本验证的数学。两层判据由数学逼出：claim (i) 拓扑，shipped builder 自己步规则诱导的 ranked labelled history 精确分布对 Kingman 闭式 `2^(n−1)/(n!(n−1)!)`，`n ∈ {3..7}` **0 mismatch**（56,700 条 history 在预算内）；claim (ii) 观测量，2101 个 replicate 的 `S̄ = 4.5307` 落在冻结验收域 `[4.4286, 4.7047]`，靶是**解析常数** `θ·H_(n−1) = 137/30`。**claim (ii) 的靶不是另一个模型的分布**——红队第五轮发现两模型 `E[S]` 在每个有限 N 上恒等，原判据"两模型分布一致"跑完必然通过却什么都没证明，是**不可证伪**的；改为对照解析靶后失败才会指向实现。载荷推导也被红队双向证伪过一次（"比值与 n 无关"既非必要也非充分，两个精确反例作为永久 check 保留），结论对而理由重写为「一步至多一次二元合并 + 条件 pair 均匀」。**P1 首次推进**：注册表原样复用 World 2 的 `second-world-observer`（对象同一性判据，挡住了手工改写 `__module__` 的复制品），复用范围由 registry 裁定（两者共同 `own TREE_PATH` 抛 `StructuralOwnershipConflictError`），无冻结产物跨世界。**如实记录**：消费方迁就了生产方的命名空间（`TREE_PATH` 是 World 2 的模块常量，第四世界状态里带 `second_world` 键，plan R2 权衡后接受），**这个组合不是干净的**；`staging.py` 第四次被绕过（goal §5.1 事前预声明，四连信号入 §8 风险清单）。世界文档见 [`docs/zh/worlds/004-moran-genealogy.md`](../worlds/004-moran-genealogy.md)。证据：`results/fourth-world/summary.json`（verdict 从合取机械计算）；实现 `mechanisms/fourth_world/` + `conform/fourth_world_verdict.py` | 判据见预注册（冻结 `24bb02e`，`prereg.sh audit` PASS）；claim (i) 五个 n 全零 mismatch，claim (ii) 在验收域内，`passed: true`；goal 的 P1 判定与 verdict 物理分开，`summary.json` 经检查不含任何 goal 词汇 |
| **第六世界（已判定）：staging 五次绕过的可归约性——回溯审计，非新世界** | **判定（2026-09-03）：H1 支持**。四个**调度相关**循环全部可由 loop unrolling 表达：World 1 `assay.py:140` `static`、World 2 `ms_coalescent.py:111` `config`、World 3 `moran.py:95` `capped`、World 4 `genealogy.py:80` `config`。**文献先行改变了这个里程碑**：「DAG 不能原生表达重复到收敛」是教科书级已知（L-DAG 2019 / J.Cloud Computing 2021 / ACM CSUR 2022），标准解法为 loop unrolling 与 recursive DAG，所以原本「证明 DAG 表达不了」的 H1 是重新发现教科书，作废；问题磨成「已知解法覆不覆盖本项目的实例」。**三条子裁定（与 verdict 分开）**：S1 次数是 **4 不是 5**（第五世界是方法学度量，无世界循环）；S2 World 1 **确为手写循环**（`mechanisms/resource_foraging/` 无任何文件引用 `staging`，`001` 那句描述的是设计意图不是实际接线，与 `002` 矛盾处由代码裁定）；S3 **「五次同因」不成立**——四个循环出现**三种** bound 类型。**真正的发现：记载指错了循环**——机械普查 64 个循环，10 个真正无界，但**无一调度相关**（全在 `ms_coalescent.py` 的拒绝采样与逆变换采样内部，属机制步内部，staging 从不需要表达）。**对 P3 的意义：`staging.py` 不需要为「重复到吸收」扩展**，并取消 §8 那条未经核对的四连/五连风险记录。世界文档见 [`docs/zh/worlds/006-staging-bypass-reducibility.md`](../worlds/006-staging-bypass-reducibility.md)。证据：`results/sixth-world/summary.json` | 判据见预注册（冻结 `671d6f4`，`prereg.sh audit` PASS）；三条负控全部按预期翻转（指向真正无界的 `while True` → H0、行号漂移 → IC-3、文件不存在 → INVALID）；分类在 verdict runner 里独立实现 |
| **第十二个里程碑（已判定）：只装声明依赖能否重放——不是世界** | **判定（2026-09-03）：H1**，5/5 逐字节复现（要求 ≥3），全新 venv 只装 `pyproject.toml` 声明的依赖（`newlife` + `proofroot`），同一解释器 3.12.8。文献公认答案是容器化（含 OS 的镜像），对手方据此押 H0——**H1 成立是因为依赖面薄得反常**。**但测出一条未声明依赖**：`second` 编译并运行 `ms` 的 C 源码依赖系统 C 工具链，不在 `pyproject.toml` 里，这次通过只因同机编译器在位。**口径连报**：「纯净」只到 Python 包这一层，同机同 OS，**不说明跨机器跨年可复现**。**⚠️ 流程违规如实记：冻结 goal 后直接跑了实验，没有单独冻结预注册**（判据确实事前冻在 goal §3，但 IC 与停止规则缺失）；**不事后补**，故 `prereg.sh audit` 对该里程碑不可用，结论强度低于其余。世界文档见 [`docs/zh/worlds/012-archival-replay.md`](../worlds/012-archival-replay.md) | goal 冻结 `42ada0e`；无预注册 |
| **第十九个里程碑（已判定）：产率是输入还是结果——产品形态的第一次端到端** | **判定（2026-09-04）：H1**，七个单元全过。问题**由积木自己长出来**：`MonodKinetics` 与 `DynamicFBA` 输入签名逐字相同、在同一端口可互换，而 dFBA 内部用的**正是 Monod 摄取律**——**共用同一条摄取律，差别只在摄取之后**。对齐不手填：Monod 的 `km`/`vmax` 运行时从 dFBA 自己的 `kinetic_params` 读出。扫描 `EX_o2_e`：氧受限 → 乙酸 1.216、产率 0.055；氧充足 → 乙酸归零、产率到 0.0863 平台——**教科书式的溢流代谢**。**唯象模型把产率当输入（恒等于声明的 `yield`），机制模型把产率当结果。** **判定缺陷（2026-09-04，外部红队触发）：Z5 是恒真的负控，判定力为零**——`[… for _ in O2_SWEEP]` 丢掉了循环变量，五次跑同一份配置；实测 `yield` 从 1e-6 到 1e3 **全绿**。**错在冻结的预注册自己**：Monod 配置里没有氧的位置，「重扫」对它不可能有效，这不是可修的实现 bug。**结论不依赖它**——Z3（dFBA 产率随氧变，极差 **56.2%**）∧ Z4（Monod 产率恒定）已由算术蕴含「对齐一个点 ≠ 表示这个自由度」。**H1 保留、产物不覆盖**（重跑 300.66s 逐字节相同），照第七世界先例登记进 `KNOWN_VACUOUS`。探测顺带挖出**产物里没人说过的事**：Monod 的表观产率与声明 `yield` 差至 **+15.3%**，Z4 的 1e-3 判据只在 `yield=1.0` 附近成立——而 `pinned_to` 与 `monod_yield_pinned` 这两个相差 15% 的数一直躺在已提交产物里，**没有任何判据去读它们**。新门 `scripts/gates/vacuous_criterion_scan.py`（具名常量 + 循环变量未用；全树 127 文件**零误报**）+ 自检 F22。**边界如实写明**：只证固定形式的 Monod 覆盖不了，不证任何唯象模型都覆盖不了。**撞出一处非确定性**：`PYTHONHASHSEED` 变了 Monod 产率末位就变（第三方路径上有 `set` 迭代）——照第十七个的办法钉住种子并记进产物；补 `flush` 前那行重启提示被 `execve` 吞掉，**重启发生了却没人看得见，那就成静默了**。**产品指标**：221 行（产物机械记录的只有这个总数；**声明 27 / 判定 158 / 哈希种子 36 的三分是人手划的、未经脚本验证**，不能拿它论证「判定层与问题无关」——上一轮实测克隆率 58/1113 ≈ 5%，方向相反），**世界侧新增 0 行**——**贵的是判定，不是世界**。世界文档见 [`docs/zh/worlds/019-yield-input-or-outcome.md`](../worlds/019-yield-input-or-outcome.md)。证据：`results/nineteenth/summary.json` | 判据见预注册（冻结 `0e04160`，audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））；goal 判 `achieved` |
| **第二十个里程碑（已判定）：复现类别的可声明性——同仓形态的第一个样板** | **判定（2026-09-06）：H0**，`S5_l2_covers_every_bundle` 为假，其余五个单元全过。21 份 bundle 中 **12 份 `unclassified`，且原因全部是 `no_runner_found`**——`rng_without_traceable_seed` 与 `mixed_seeded_and_stochastic` 各零份。**三类划分在能定位到源码的 9 份上全部命中，没有一份落进「特征表不够用」**：H0 量的是「从目录名猜 runner 文件名」这个辅助启发式的能力，不是被研究的那个划分。**这一点能被一眼看出，靠的是预注册约束 6 要求`unclassified` 的原因取自固定值域、由代码算出**——若由散文写，这里就会变成事后找台阶（第十八个的教训）。**对手方的论证被定量化**：只读产物（L1）分类 21/21、`unclassified` 为 0，看起来覆盖率 100%，**那是没有能力发现问题，不是分得清**；加上 runner 源码（L2）才暴露出 12 份。`second-world` 在 L1 判 `deterministic`、L2 判 `seeded`。**零份 `stochastic`**，与 §0 记下的先验一致。**产品产出（goal 判 `achieved`）**：`prereg.md` 模板新增 `@reproduction_class` 声明，`newlife-prereg` skill 新增**规则七**——`seeded` 世界必须另加一条「换一组冻结种子 verdict 不变」的判据（**绿 S0 只证明种子被固定，不证明结论换个种子还成立**），`stochastic` 世界须在记录上豁免 S0 并按分布判定。规则七第二半来自当日实测：一个「模拟器」是远端 LLM 的 runner，`temperature=0` 下**S0 绿、S1 绿、audit 五臂全 PASS**，而在 `verdict.py` 一字未改的前提下换掉模型，数据从 `[-10.0, -1.5, -1.2, -1.25]` 变成 `[-1.0, -1.0, -12.5, -12.5]`——**每一道检查都是绿的，而决定答案的东西没有任何地方记录**。**形态首创**：预注册与产物同仓，`newlife audit` 五臂全 PASS，**CHRONOLOGY 第一次真正成立**（此前二十份跨仓注册全部只能报 PARTIAL）。**行数估计偏低 74%**（估 300、实际 522），漏算的成分正是 skill 里已有的那条——「同一逻辑按纪律写两遍」。**一个未解决的机制冲突**：skill 要求收尾判定写进 goal 的小节，而 `@frozen` 对整个文件算哈希，填了就必然失效；本轮处置是 goal 保持冻结原样、收尾判定落在世界文档，**正解应是让 `@frozen` 只覆盖被保护的章节**。世界文档见 [`docs/zh/worlds/020-reproduction-class.md`](../worlds/020-reproduction-class.md)。证据：`questions/2026-09-06-reproduction-class-declarability/results/summary.json` | 判据见同仓预注册（冻结 `876ed29`，audit **PASS**，含 CHRONOLOGY）；goal 判 `achieved`（冻结 `1b0a113`） |
| **第二十一个里程碑（已判定）：不确定世界的可判定性——规则七的建造那一半** | **判定（2026-09-06）：H0**，`S5_across_models_not_equivalent` 为假，其余六个单元全过。**H0 的位置出乎意料**：失败的不是「4b 与 9b 分不开」（那是对手方押的），而是**样本量停在了 8**。可从产物复核：`A2/-4.0` 含一个 `-400` 离群值，sd=138.28，该点 CI 宽 **159.4**，而两模型均值差约 2–3——**效应量是区间宽度的 2%**，任何两组 8 样本都会判等价。**本轮最有价值的发现是收敛判据的方向是反的**：方差越大 → CI 越宽 → 两半越容易判等价 → 越早判「收敛」，**它在最需要更多样本时最早喊停**，把「区间宽到什么都含 0」误读成「样本已稳定」。**这个缺陷只有在 S5 失败时才暴露**——若 S5 侥幸通过，n=8 会被当作高效的证据。**H0 买到的比 H1 多。**下一轮规格由此确定：收敛判据必须与**效应量**挂钩，而非只问两半是否等价。**三条方法学记录**：① **写两遍防不了设计错误**——「一对同分布样本必须判等价」这个错误形状在 `equivalence.py` 改掉后，又在刻意独立实现的 runner 里原样重犯，由 pilot 拦下；约束防的是实现错误，而两份实现出自同一个头脑。② 多重比较在首次 selftest 即被抓住（两点各 95% 的合取约 90%），已加 Bonferroni，**但校正后实测假警报率仍约 10%**（0.875–0.925，40 次试验，百分位 bootstrap 小样本覆盖不足），该数**冻结前**测得写入 §0，故 S4 为真的证据强度须按此折算。③ **S0 被显式降级**：本轮只证明判定层不引入新的不确定性，不证明世界可复现。**规模低估 39%**（估 390、实际 541），与上一轮同因——「独立实现的第二份」仍未计入成分清单。**形态**：同仓，audit 五臂全 PASS；**goal 是第一份带 `@frozen_end` 的冻结文档**，收尾判定填入 §7 而冻结哈希不变（已实测），第二十个里程碑暴露的「冻结与收尾互斥」到此闭合。世界文档见 [`docs/zh/worlds/021-stochastic-world-judgeable.md`](../worlds/021-stochastic-world-judgeable.md)。证据：`questions/2026-09-06-stochastic-world-judgeable/results/summary.json` | 判据见同仓预注册（冻结 `994c4bf`，audit PASS）；goal 判 `achieved`（冻结 `56dc2b0`） |
| **第二十二个里程碑（已判定）：判据设计方法论——产出是方法不是判据** | **判定（2026-09-06）：H0**，唯一失败的 S5 是**采样预算**，不是方法论失灵。用户定的形状：「规则给不出来」——问题、数据、算法都不同，重复几次、用什么判据不可能一样，所以产出是**六个冻结前必答项**加「答不上来就冻不了」。**方法论对两个世界给出由数据决定的、实质不同的答案**：尾比 3.77（重）→ `trimmed` → **64 次调用内判不了**；尾比 1.17（轻）→ `mean` → **n=16**。**S6 做到了上一轮做不到的事**——同一个阈值两端同时满足（稳定性 CI `[-1.312, 0.750]` 含 0、敏感性 CI `[-5.500, -2.312]` 不含 0，中心 −3.9 与溯祖理论的 2.59 量级一致）。**S7 真**：统计量与 N 两项都不同，方法论不是伪装成方法的常量。**H0 比 H1 更有说服力**：若 S5 也过，方法论会显得万能；现在它划出实测边界——重尾 ＋ 昂贵采样 ＝ 此预算下判不了，并顺带给出资源需求。**数学**：不用 `n=2(z+z)²σ²/δ²`（假设正态等方差、描述 t 检验，三者皆不成立），改用**自举功效分析**，每步跑与判定完全相同的检验；**顺序是形状→统计量→N**——普通 bootstrap 在重尾下估计样本均值的分布会失败，实测重尾数据上 `trimmed` 需 64 而 `mean` 在 256 预算内达不到。**格式经自我评审整个换掉**：单行锚（多观测量装不下、遇 `>` 静默消失、理由无处展开）→ `judgement-design.json` ＋ `--data` 钉哈希。**自我评审六条修四条**：判定形式须声明（点名列出不支持的形式，**含第二十一个里程碑自己的趋势问题**）· iid 无出口（AR(1) φ=0.85 实测 0.74 对 0.80）· pilot 充分性循环 · 重尾自报改交叉核对。**规模低估 114%**，原因与前两轮不同——漏的是**自我评审引出的返工**，教训是把它当独立成分而非误差。**产品产出**：规则七补上「N 从哪来」与明确适用边界，配 `gates.judgement_design` 两层检查。世界文档见 [`docs/zh/worlds/022-judgement-design-method.md`](../worlds/022-judgement-design-method.md)。证据：`questions/2026-09-06-judgement-design-method/results/summary.json` | 同仓预注册（冻结 `55feaeb`，audit 五臂全 PASS，DATA 验 3 份哈希）；goal 判 `achieved`（冻结 `f3b9b19`） |
| **判定层的共用断言（工具，不是里程碑）** | 架构回顾里我说五个 runner（十四–十八）有「约 1100 行同构代码」，建议抽一个判定层 harness。**实测 58 行 / 1113 行 = 5%，错了近 20 倍**——形状押韵 ≠ 代码同构，五个 runner 的骨架很薄，主体是各自的科学，**P3 的类比在判定层不成立**。（第一次测量本身也错了：`ast.unparse` + **逐行** difflib 把 return 字典压成一行，三个几乎一样的函数报出 0%——**和 Y1 子串检查同一个病：粒度选错**；改字符级后才得到真实的 67%–99%。）**所以没建 harness**，只做 `conform/judgment.py`——三条断言（基线未变 / 第三方未被改动 / 对照组不含 newlife）的**单一定义处**。抽取理由不是行数（整体还多了 33 行）而是**判据不该有三份定义**；实测确实烂了一处：第十五个的 F3 用 `line.startswith(...)` 没先 strip，**漏掉函数内缩进的 import**，而 `bare_control.py` 正好有函数内 import。**只共用定义，不共用上报形状**——四个 safety line 报的键各不相同，塞进一个带选项的共用函数就是 inner-platform effect 的第一步。**五份已判定产物逐字节不变**，这是行为等价的唯一硬证据。五个 runner 1113 → 1039 行 | 无 verdict（工具改动）；证据：五份产物 sha256 不变 + 五个 runner 全部 exit 0 |
| **第十八个里程碑（已判定）：权限声明能不能机械推导——不是世界** | **判定（2026-09-03）：H0**。三次接第三方（十五 / 十六 / 十七）成色都被降级，理由是同一句：第三方交付了类型词表、模型与参数、连接线，**但没有一个说过「哪条路径归我、我只发这类 Effect」**。**必须人写，接第三方就无法规模化。** 判据 C1 要求推导声明与三份手写声明**逐项相同**；`Y3`（`claims`）1/3，故 H0。**但拆开看是好消息**：`Y4 allowed_effects` **3/3** · **`Y5` `add`/`set` 算符 3/3** ← 类型-效应系统的文献点名说「capability 提供的是类型本身给不了的东西——细粒度、依赖上下文的权限」，**算符正是那一类，三个全推对了**。推法是**行为探针**：不问类型名，问那个 store 类型 `apply(schema, 1.0, 0.5)` ——`PositiveFloat`→1.5（求和→`add`）、`SetFloat`→0.5（覆写→`set`）；`Y2` 每次跑前验探针真能分辨，否则整套推导无判定力。两处 `claims` 差异经**机械分类**（代码算的，不是散文判断）**都不是信息缺失**：`Grow` 是 **`layout`**（差的是**我们**在第十五个里程碑自己选的状态布局），`DynamicFBA` 是 **`finer`**（**推导更细**——三条底物各自的路径；手写当初被端口粒度限制成父路径，即第十七个里程碑记下的那处损失），`MonodKinetics` `identical`。**`underivable_items = []`——零项推不出来。** 接线**真从第三方拿**：三个生成器形状各异（顶层节点 / 嵌套在 `_value` 下 / 裹在 composite 里），**一个递归遍历器吃掉三种**，第四个第三方来了不用加代码；`Y1` 用 **AST** 检查推导模块没引用任何 `declaration` 常量。**一处自打的脸**：`Y1` 第一版按**子串**查，命中的是 `derive.py` 文档字符串里「本模块不 import declaration」那段散文——**检查在自己的说明文字上误报**，与「检查太弱」是同一个病的反面：**检查太笨**，断言的对象必须是**结构**不是文本。**H0 之后两条可执行修正**：① 第十五的世界改用第三方给的状态布局；② `admit()` 改为**按 key 逐条发 `StateDelta`**。**两条做完，权限声明就不再需要人写。** 规模 489 行 vs 事前 465（**+5%**）——修正后的校准第三次落在 ±5% 内。世界文档见 [`docs/zh/worlds/018-derive-authority.md`](../worlds/018-derive-authority.md)。证据：`results/eighteenth/summary.json` | 判据见预注册（冻结 `f948447`，audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））；goal 判 `not_achieved`，但明写 H0 的内容与两条修法。**【2026-09-04 后续】两条修法已做完，同一个 runner 判 H1**：① 第十五个改用第三方自己给的状态布局（原来的 `cell/` 前缀是我们凭空加的，第十五仍判 H1，产物 `a4f92066…`→`b83f33fa…`）；② 推导塌到接入路径的粒度上限——**理由取自 `admit()` 的代码**：它对每个端口只发一条 `StateDelta`，而一条只有一个路径，所以按 key 接线的端口无法声明到每个 key 各自的路径上；推导给出更细的路径，产出的是一份 `admit()` 消费不了的声明。**两条都不是「改手写声明去迁就推导」。**结果：**Y3 1/3 → 3/3，Y6 通过，verdict H1**——**三个第三方 provider 的权限声明现在全部可机械推导，不需要人写。** **判定之记录仍是 H0**（预注册 `f948447` 下，产物 `bae0508a…`），产物里的 `judgment_of_record` 字段把两件事分开写，**不因后来的修复而改写判定**。**没做的那一半**：解除粒度上限要让 `admit()` 按 key 逐条发 `StateDelta`，而按 key 接线**只有一个 provider**——少于三个不许定型。**副产物**：安全绳形成链条，这次改动**四轮才收敛**——对已判定输入的刻意改动会让下游立刻开火，而不是悄悄带着旧基线继续绿 |
| **第十七个里程碑（已判定）：一个 LP 求解器背后的第三方 process——不是世界** | **判定（2026-09-03）：H1**，八个单元全过。接的是 `spatio_flux.processes.dfba.DynamicFBA`（`spatio-flux==1.4.0`，E. coli core = cobra 自带 `textbook` 模型，**不联网**），**源码零改动**。它测的不是第十六个那件事：**`MonodKinetics` 的确定性来自代码，`DynamicFBA` 的确定性来自求解器。** X0 安全绳（第十五 + 第十六产物均不变）· X1 未被改动 · X2 与**裸 pb**逐字节相同 · X3 正控（生物量增长、葡萄糖消耗、乙酸分泌）· **X4/X5 负控加了一条前两次没有的检查：异常必须来自 `newlife.core.errors`，来自 `cobra`/`optlang` 的不算**（求解器背后什么异常都会抛，「抛了异常」≠「契约拦住了」）· X6 元负控 · **X7 求解器身份进合取**。**H1 之下连报三件事**：① **逐字节复现买到的是「同一个求解器的同一个选择」，不是「同一个科学答案」**——FBA 最优解通常不唯一（问题退化，FVA 存在正是为了刻画这个范围）；换求解器/换 GLPK 版本这条轨迹没有理由仍逐字节相同，**本轮未测**。② **GLPK 是第三条非 Python 依赖**，第十六个里程碑装 `spatio-flux` 时顺带带进来、**无人提醒**；**没有改动第十三个里程碑冻结的那份穷举**（它是对当时那份代码的穷举），新增 `DISCOVERED_SINCE` 并写明**第十三个的 PATH 屏蔽实验够不着共享库**、本轮没做。**穷举有有效期。** ③ **端口的粒度限制了声明的粒度**：按 key 接线时一条 `StateDelta` 只有一个路径，`substrates` 只能声明在父路径上，比第三方实际写的位置粗——不是实现偷懒，是接法本身的上限。**适配器只改一处，但那是同一个坑的第三层**：十五以为一张接线表够、十六拆成读写两张、十七发现表里的**值**也不只一种形状（可以是 `{key: 路径}`）。**三次接第三方下来，我们代写的部分一直是同一样：权限。** 第三方交付的越来越多（十六给类型词表，十七还给模型与参数），但**没有一个第三方说过「哪条路径归我」**。规模 416 行 vs 事前 400（**+4%**）——修正后的校准很准。世界文档见 [`docs/zh/worlds/017-solver-backed-process.md`](../worlds/017-solver-backed-process.md)。证据：`results/seventeenth/summary.json` | 判据见预注册（冻结 `642361c`，audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））；goal 判 `achieved` 但明写三件必须连报的事 |
| **第十六个里程碑（已判定）：一个真正独立的第三方包接得进来而契约还咬得住吗——不是世界** | **判定（2026-09-03）：H1**，七个单元全过。接的是 `spatio_flux.processes.monod_kinetics.MonodKinetics`（**`spatio-flux==1.4.0`，独立发行的第三方包**，走 optional extra），**源码零改动、不子类化覆写 `update`**。W0 安全绳：第十五个里程碑产物逐字节不变 · W1 仍定义在 `spatio_flux` 且位于 site-packages · W2 与**裸 pb**的 `mass`/`exchange` 两条轨迹逐字节相同 · W3 正控（生物量增长、葡萄糖被消耗）· W4/W5 负控被拒且状态不变 · **W6 元负控：同一份错误声明绕开契约则不被拒**。**第十五个里程碑的结论收回一半**：当时只查两个候选就说「生态不通」，**搜索太窄**。系统找一轮的结果：`biosimulator-processes` 0.3.19 ✗ · `vivarium-interface` 0.0.5 ✗ · `viva-munk` 0.0.2 ✗（**未声明依赖** `spatio_flux`）· **`spatio-flux` 1.4.0 ✓**（5 个模块 11 个 `Process`/`Step` 全部可用，含 `dfba`）。**三个失败包有共同形状：都没把自己的依赖说清楚，安装静默成功、import 才炸。****撞出三处，第二处是真发现**：① `state_roots` 假定顶层值是映射（`mass` 是标量）；② **`lowering_table` 按 Effect 类别做键 → 一个机制每类 Effect 只能写一个端口**——`MonodKinetics` 写两个，两条 `StateDelta` 全落到同一端口，`mass` 收到一个 dict；**单端口的 `Grow` 看不出，两个端口就塌**，第九世界那条教训落在降级表上的又一次。改为先按 `(类别, 路径)` 找、再退回只按类别，向后兼容，`fourteenth`/`fifteenth` 逐字节不变；新增 `sum-map-add` handler。③ 读写异路径（`substrates` 读 `local` 写 `exchange`），接线表拆成两张。**import-lint 第三次咬人**：`spatio_flux` 加进 vendor 白名单，**显式声明而非文件豁免**。**成色（预注册 §5 事前声明）**：`spatio-flux` **交付了类型词表**（`register_types(core)`，对应本体文献「本体在独立组件之间充当接口契约」），**词表这一半不用我们代写了**；但**权限声明仍由我们代写**——路径归属、允许的 Effect、增量还是覆写，第三方 `outputs()` 一个字没说。**词表来自第三方，权限仍由我们代写。**规模 389 行 vs 事前 475（**−18%**）——**新校准「每项加三成」第一次用就过头**：成分列全得 365、实际 389，原始估计只差 +7%。世界文档见 [`docs/zh/worlds/016-external-package.md`](../worlds/016-external-package.md)。证据：`results/sixteenth/summary.json` | 判据见预注册（冻结 `588137e`，audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））；goal 判 `achieved` 但明写成色保留 |
| **第十五个里程碑（已判定）：一个第三方 process 接得进来而契约还咬得住吗——不是世界** | **判定（2026-09-03）：H1**。接的是 `process_bigraph.processes.growth_division.Grow`，**源码零改动、不子类化覆写 `update`**。V1 它仍定义在 vendor 包内 · V2 经 newlife 接入的轨迹与**裸 pb**（不 import 任何 newlife）逐字节相同——**包装是忠实的** · V3 正控增长正确 · V4 代写声明错路径 → `CommitAuthorityError` 且状态不变 · V5 未声明 `StateDelta` → 同上 · **元负控**：同一份错误声明绕开契约则**不被拒**，证明拒绝确实是契约干的。**先说负面结果**：已发布的 Vivarium-2 生态入口包 `biosimulator-processes` 0.3.19 与 `vivarium-interface` 0.0.5 **都装不起来**——都 import `ProcessTypes`，而它在任何能与 `bigraph_schema` 1.6.0 配对的 pb 版本里都不存在（实测 1.8.3/1.8.2/1.8.0/1.7.1/1.5.0/1.4.18），且**两者都没声明 pb 版本下界**，安装静默成功、import 才炸。**所以改用 pb 自带的领域 process。****成色降级（预注册 §5 事前声明）**：端口→(路径,算符) 那张表只能由我们代写，第三方两处信息都不提供；对照 FMI——那里接口描述由模型作者随实现一起交付。**H1 证明的是「契约对被代写的声明有强制力」，不是「第三方生态可以安全地接」。****判定录完之后补了一道守卫**：把代写算符从 `add` 填成 `set`，修复前**产出完全相同的轨迹、无人报警**；丢失在 store handler（`_sum_float_add` 假定 ADD 却不校验，而同仓库 `_budget_proposal_projection` 等两处本来就有这个校验）。已补齐 5 个 handler，四份已判定产物修复后逐字节不变。**顺序刻意：先录判定，再修。**今天的边界：填一条**无人认领**的路径，契约不会说话。规模 449 行 vs 事前 345（**+30%**，从上次 +39% 收窄）。世界文档见 [`docs/zh/worlds/015-foreign-process.md`](../worlds/015-foreign-process.md)。证据：`results/fifteenth/summary.json` | 判据见预注册（冻结 `8d65582`，audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））；goal 判 `achieved` 但明写成色降级 |
| **第十四个里程碑（已判定）：同一个世界跑在两个运行时上，结果一样吗——不是世界** | **判定（2026-09-03）：H1**。U0 安全绳 PASS（harness 改收 runtime 工厂后 RK 产物逐字节不变）· U1 pb 跑完 2101/2101 · U2 **2101/2101 逐字节相同** · 负控变红 · 6.8s。**process-bigraph 从零使用变成真跑通一个世界**，752 行适配终于有了第一个用户。两个负控各有信息：改坏 pb 写树 → `segsites` 9→0（证明 pb 的写入在关键路径上）；**颠倒声明里的阶段顺序，pb 照样跑对（按接线依赖定序），RK 直接抛 `TypeError`（按列表顺序）——pb 在这一点上严格更强**，这是唯一一条「留着 pb」的正面证据。**最重要的产出不是 H1，是事前未预见的那条**：`core/runtime.py`（B 的 Definition）的四个操作是从 `ReferenceKernel` 读出来的**逐阶段 pull**，而 pb 是**先拿整张图再自己调度的 push**——`adapters/process_bigraph/world_runtime.py` **不是 `WorldRuntime` 的实现**，是 `WorldSpec` 的第二个消费者。**B 接缝今天仍只有一个 provider 满足它的 Definition**，现在的形状已知是错的（把 RK 的 pull 当成了通用形状），下次该规定的是**图与依赖**。顺带：架构图标红的第①条闭合；依赖声明自第八个里程碑起陈旧（已补）；**第七世界判定记录自第八个里程碑起是烂的**（4 个 phantom 部件全是被删的 `MoranGenealogyWorld.*`，重跑得 INVALID 而非记录的 H0）——`verdict_rot.impacted_by` 回溯适用正确抓住了它，当时没人跑；`test_reuse.py` 5 个测试同源一直红。**未覆盖那份已判定产物**，单独立项。规模 460 行 vs 事前 330（**+39%**，超出近五次的 ±25%）。世界文档见 [`docs/zh/worlds/014-cross-runtime-verification.md`](../worlds/014-cross-runtime-verification.md)。证据：`results/fourteenth/summary.json` | 判据见预注册（冻结 `1b9257a`，audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））；goal 判 `achieved`，verdict 判 H1，但 goal §6 明写「只买到一半」 |
| **第十二个里程碑（已判定）：只装声明的依赖，冻结产物还重放得出来吗——不是世界** | **判定（2026-09-03）：H1**。全新 venv，**只装 `pyproject.toml` 声明的依赖**（`newlife` + `proofroot`，别无他物），同一解释器 3.12.8：`second`/`third`/`fourth`/`sixth`/`ninth` **5/5 逐字节复现**（判据要求 ≥3）。文献的公认答案是**容器化**——「除非依赖被版本化且运行在一致环境中，否则不保证确定性输出」，对手方据此押 H0；**这次 H1 成立是因为依赖面薄得反常：声明的依赖只有一个**。**同时测出一个未声明的依赖**：`second` 编译并运行 `ms` 的 C 源码，依赖系统 C 工具链，而那不在 `pyproject.toml` 里——**换一台没有编译器的机器，归档复现会失败**（这条直接催生了第十三个里程碑）。**⚠️ 本里程碑无独立预注册**：判据事前冻结在 goal §3（`42ada0e`，早于建环境与跑数），但 IC 条件与停止规则缺失，**不事后补**——补一份就是这套机制要防的那件事；故 `prereg.sh audit` 对它不可用，**结论强度低于前十一个**。**⚠️ 产出者未提交**：本条产物无法重跑，**与它自己的主题（归档重放）矛盾**，已在 `scripts/check_record.py` 的 `KNOWN_GAPS` 里显式登记，不伪造一个产出者去凑绿。**本行是 2026-09-03 由对账器查出缺失后补上的**——收尾清单里专门有一条防孤儿文档，**照样漏了**。世界文档见 [`docs/zh/worlds/012-archival-replay.md`](../worlds/012-archival-replay.md)。证据：`results/twelfth/summary.json` | 判据见 goal（冻结 `42ada0e`）；无独立预注册 |
| **第十三个里程碑（已判定）：非 Python 依赖缺了会不会静默——不是世界** | **判定（2026-09-03）：H0 支持**。`pyproject.toml` 之外穷举出**两条**：`cc`（`ms_binary.py:19`）缺失时 **`hard_fail_named`** ✓ 报 `FileNotFoundError: 'cc'`；**`git`（`verdict.py:315` 等三处）缺失时 `silent_output`** ✗✗——`eighth_world_verdict.py` 里的 `try/except Exception: pass` 吞掉异常，**产出一份格式完好的 `verdict: INVALID`，理由写成「基线不可得」，而真实原因是 git 不在**。**不是没报警，是报错了案由。** design §2.3 在跑之前就点名了这一处嫌疑，实测证实。对照组（`third`）两次判 `unaffected`，证明分类不会把「用不到」误记成「装作没事」。**本项目第四次栽在同一形状**（前三次：mutation_scan 空集恒真、gate_coverage_check 的 `if m else 0`、verdict_rot 跨仓库解析失败当成没变动），**而这次在环境层，`silent_degradation_scan` 完全够不着**。穷举边界如实写明：只覆盖已知会被执行的路径，声明的是「已知需要什么」不是「只需要这些」。世界文档见 [`docs/zh/worlds/013-undeclared-deps.md`](../worlds/013-undeclared-deps.md)。证据：`results/thirteenth/summary.json` | 判据见预注册（冻结 `6c94455`，audit：FROZEN/STAMP/INTEGRITY 通过，**CHRONOLOGY 未建立**（见 §7 注））；goal 判 `achieved` 而 verdict 判 H0——**G4 成立：H0 之下 goal 仍达成** |
| **第十一个里程碑（已判定）：八个旧判定还有几个是坏的——不是世界** | **判定（2026-09-03）：H1 支持**。除已知的 `seventh` 外，**七个全部逐字节复现**（复现 7 · 坏 0 · 跑不了 0）。**这个数要连口径一起报**：文献的复现衰减基准率很难看（27,271 个生物医学 notebook，依赖装得上的 10,388 个里只有 879 个产出相同结果，约 3.4%；IR 研究发现四年后多数已无法运行），**但那是依赖漂移、跨年发生；本项目是同一台机器、同一环境、几天之内，唯一那次失效也是我们自己的第八个里程碑造成的**。所以 7/7 说明「环境不变时冻结判据 + 确定性种子撑得住」，**没有说明能扛住跨年漂移——那从未被测过**。**起草期做对的一个区分**：归档复现（`git checkout` 冻结 commit + 冻结种子，**能**保障）与当下有效性（今天的代码是否仍满足当时判据，**不能也不该**保障）是两个问题，本里程碑测第二个。**四态而非二态**：`reproduced` / `value_changed`（答案变了，**系统演化是正常科学结果不是故障**）/ `unevaluable`（**问题没了**，判据引用的对象已删）/ `unrunnable`（不计入 H0）；两者混淆列为 IC-2。**顺带测到的归档缺口**：8 份产物里 6 份记了自己的冻结 commit，`second` 与 `fifth` 没记，归档复现对这两个不是一条命令。世界文档见 [`docs/zh/worlds/011-verdict-rot.md`](../worlds/011-verdict-rot.md)。证据：`results/eleventh/summary.json` | 判据见预注册（冻结 `3a2ca54`）；负控三态全部实测翻转（改一字节 → `value_changed`/H0、加一行 raise → `unrunnable`/INVALID、`seventh` 实时演示 `unevaluable`），恢复后回 H1 |
| **第十个里程碑（已判定）：判定这道接缝，从八个已有实现里提取——不是世界** | **判定（2026-09-03）：INVALID**，触发一条预注册没预见到的 IC。C1（三个 runner 都经由同一Definition）与 C2（值域仍是三值、未放宽，机械查 `DOMAIN`）**全过**；C3 逐字节不变 **2/3**。**三代表示确实装进了同一个 Definition**，靠把被同一键名混住的两件事分开命名：**判定是三值，假设名是标签**，标签由声明式映射给出（`{H1: "ms_minimal_coalescent_reproducible", H0: "verdict_withheld"}`），映射须对值域穷尽——`second` 的 runner 里因此不留 `if/else`。**IC-4 的裁定**：`seventh` 对不上冻结基线的原因**不在接缝**——第八世界删去 `fourth_world/world.py` 的 4 个部件，第七世界的分类表仍声明它们 → `phantom:4` → verdict 由 H0 变 INVALID；**把改动收起来跑原版，原版给出同样的 INVALID**，且原版 vs 接 Definition 后逐字节相同。按字面 C3 不成立即 H0，但 **H0 的含义是「Definition 装不下」而这里装得下**，照字面判会把「第七世界产物过期」误记成「接缝装不下第四代表示」——**预注册没覆盖的情形不许硬塞进 H0/H1**。**最值钱的产出：第七世界的判定已不可复现，而此前没有任何东西会发现它——因为没有任何东西在重跑旧世界的 verdict。这正是「判定该有接缝」的最强论据。** 世界文档见 [`docs/zh/worlds/010-verdict-seam.md`](../worlds/010-verdict-seam.md)。证据：`results/tenth/summary.json` | 判据见预注册（冻结 `a517d29`）；负控：把标签映射改成常量 → `second` 产物立刻不同，恢复后逐字节相同，证明 Definition 真参与产出 |
| **第九世界（已判定）：真 tick 循环的终止条件能否纯数据表达——补第八世界未检验的那一半** | **判定（2026-09-03）：H0-a 支持**。C1 失败、C2 通过、C3 未评估。**正面结果（C2）：终止条件确实能用冻结算符集表达**——`tick lt config.ticks` ∧ `population gt 0`，两个子句、只有合取、算符在冻结的六个比较关系内、observable 是已声明观测量名（非任意状态路径），配置经 AST 检查为纯数据。**没有为让它通过而扩算符集**——那是预注册点名禁止的 inner-platform effect 的第一步。**失败的两个原因都是结构性的**：① **`run` 是混合部件**——21 行里只有中间的 `while` 属 `simulator`，前后是 `frame`（tick-0 快照 + `WorldRunResult` 聚合）；只吃循环则 `run` 不消失，连带搬走则超范围，**两条路都是 H0-a**。**这反过来打了第七世界一记：它把 `run` 整体判为 `simulator`，归类粒度是「函数」而真实类型边界在函数内部。** ② **装配所有权冲突**——通用 harness 假设自己拥有引擎装配，World 1 已经拥有；接线只能写成 `GenericWorld.__new__` 再手填字段，**那个写法本身就是证据：harness 是并排贴上去借一个循环，不是吃掉**。**结论：卡住的不是表达能力，是部件边界与装配所有权**——下一步不该扩配置语言。**C3 未评估**（重跑 v0.2 gate 需外部 L2 录制根目录，本次不可得；合取在 C1 已断，不改变 verdict——但未评估就是未评估）。**goal 判定 `not_achieved`，并自我批评：达成判据与 H1 逐条重合，违反 G4，冻结时没看出来；不许事后改窄判据让自己通过。** 世界文档见 [`docs/zh/worlds/009-harness-tick-loop.md`](../worlds/009-harness-tick-loop.md)。证据：`results/ninth-world/summary.json` | 判据见预注册（冻结 `10e26c7`）；中途叫停一次 debug 螺旋——继续调是在为不可能改变结论的度量烧预算 |
| **第八世界（已判定）：通用 harness 把 World 4 吃到零——建造，P3 首次成立** | **判定（2026-09-03）：H1 支持**。三条判据全过：第七世界判为 `simulator`/`frame` 的 4 个部件在 World 4 侧**全部消失**（AST 枚举，挪到别处也算没消失）；配置是**纯数据**（无 lambda / 函数定义 / eval 类构造，AST 检查）；verdict **逐字节不变**（`7e5bbc47…` 前后相同）。账面：`world.py` **148 → 68 行**（只剩 `model`），新增纯数据配置 `spec.py` 62 行 + 通用件 `core/harness.py` 170 行——**通用件是一次性成本，配置是每世界的常量成本**。**逃生口没有出现**：预注册冻结了「若终止条件或观测提取只能用 lambda 表达即判 H0」（框架文献恰恰不处理「模型装不进抽象时怎么办」），原 `run()` 的两个 lambda 闭包被拆成阶段顺序 / 静态参数 / 命名流绑定三样纯数据。**P1 未被破坏**（`observer_is_world2_object` 仍为真，由逐位不变一并保证）。**边界（事前声明，非事后免责）**：World 4 的 `run` **没有循环**，`simulator` 块最难的一半（真 tick 循环）**本次完全未检验**，「P3 成立」的正确读法是「吃得下无循环的两阶段世界」；数值复现与运行时契约强制两块用不到、也未检验。**一次实现 bug 按 IC-1 处理**：verdict runner 首版从预注册 freeze commit 取基线，而那个 commit 在 exloop、文件在 newlife，跨仓库解析不了，修完重跑不计 H0。世界文档见 [`docs/zh/worlds/008-harness-absorbs-world4.md`](../worlds/008-harness-absorbs-world4.md)。证据：`results/eighth-world/summary.json` | 判据见预注册（冻结 `6f2bc25`，`prereg.sh audit` PASS）；三条负控全部按预期翻转（配置塞 lambda → C2 FAIL、放回一个部件 → C1 FAIL、`mean_segsites` 改 1e-12 → C3 FAIL）；**NC-3 第一版是无效变异**（按显示值 `4.5307` 替换，而 JSON 是全精度），与「`return P*2` 作用在列表上」同类，已重做 |
| **第七世界（已判定）：六个世界的 harness 有多少能生成——回溯审计，冲 P3** | **判定（2026-09-03）：H0 支持**——42 个顶层部件（872 行）里 **4 个落不进冻结词表**。词表取自先验知识而非自拟：Zeigler 的 DEVS 三分（model/simulator/experimental frame）+ 本项目补的 `rng`；**预注册明写「落不进不许扩词表，直接判 H0」**——扩词表就能永远 H1。分布：`simulator` 21 · `frame` 12 · `rng` 3 · `model` 2 · **`unclassified` 4**。**落不进的 4 个聚成两种能力**：**数值复现**（3 个，复现 Julia 的 SIMD 浮点求和顺序——任何以「逐位复现另一实现」为判据的世界都会长出这类代码）与**运行时契约强制**（1 个，`_validate_world_invariants`；**这正是痛点 P2，而 DEVS 三分不含它，因为 DEVS 不关心谁有权写什么**——这不是 DEVS 的疏漏，是本项目相对经典模拟架构多出的一层）。**产出即通用 harness 的规格：五块不是三块**，其中最大的 `simulator`（21/42）正是 `staging.py` 本该覆盖的地方——三个世界各自手写了一遍 `_run_stage`；`frame` 12 个部件**目前完全没有对应物**。**World 3 裁定（与 verdict 分开）**：它根本没有 harness——不装配引擎、不注册机制、verdict runner 直接调机制函数；**是 P3 唯一「成立」的实例，但代价是完全绕开 registry 与引擎，不计入 P3 进度**。世界文档见 [`docs/zh/worlds/007-harness-generability.md`](../worlds/007-harness-generability.md)。**【2026-09-03 补记：产物腐烂与处置】** 本判定写于 `7e3b7a5`；`6bbc05c`（第八个里程碑）从 §3.2 的 `mechanisms/fourth_world/world.py` 删掉了 `MoranGenealogyWorld`——那是它的成果。重跑得枚举 38 / 声明 42、4 个 phantom → **INVALID**，**即原判定 §5 第三条负控「声明幽灵部件 → INVALID」被非自愿触发，机器在正常工作**。`verdict_rot` 把它归为 **`unevaluable`**（判据引用的对象已不存在），`impacted_by(['mechanisms/fourth_world/world.py'])` 回溯运行**正确返回含 `seventh`**——工具抓得住，缺的是**收尾时跑一次**（已补进 skill 收尾清单）。**处置：保留 H0，不编辑 `classification.json`，不覆盖产物。** 四条理由均本次实测：① 预注册 IC-3 的原话是「§3.2 的文件在**判定前**被改动」，本次改动在判定**之后**；② 4 个 `unclassified` **全在** `resource_foraging/world.py`（第八个里程碑没碰），被删的 4 个当初归类是 `simulator`×3+`frame`×1，**稳健性探测（仅探测、不入 `results/`）去掉已不存在的部件后重跑仍是 H0、同样那 4 个**；③ **用 `7e3b7a5` 当时的 runner 跑今天的代码，与新 runner 逐项一致**，第十个里程碑的 D 接缝改动被排除；④ 预注册 §3.3 明写「归类**不预先冻结**」，`classification.json` 是判定时的声明快照，改它是**另做一次实验**；且 §3.4 的 42 明写「可行性测量，非确证运行」，**部件数从来不是结论**。**覆盖成 INVALID 会毁掉一份有效发现**——`INVALID` 是「这次运行没有判定力」，而 `7e3b7a5` 那次有；正确的词是 **`unevaluable`：不能再被重新评估 ≠ 当初判错了**。顺带查出 `verdict_commit` 把产物路径写死成 `results/{world}-world/`，使 `fourteenth`/`fifteenth` 一直在陈旧扫描的视野外——**硬失败救了它**，已修。证据：`results/seventh-world/summary.json` | 判据见预注册（冻结 `c10dcb3`，`prereg.sh audit` PASS）；四条负控全部按预期翻转，含「偷偷用词表外取值 → INVALID」；verdict runner 独立重做枚举，不读 design 阶段台账 |
| **第三世界（已判定）：Moran 选择可判定性，兼证据分层裁定** | **判定（2026-09-02）：H1 支持**。冻结网格 N=6、i₀=1、r ∈ {1/2,1,3/2}、with-replacement、birth-first，4050 个 replicate。claim (i)：**每一步**由独立 oracle 从 `(pre, draw1, draw2)` 重放，逐值零容差，全部通过；claim (ii)：三个 cell 的不动次数 23/232/494 全部落在精确二项验收域 [11,33]/[193,258]/[451,536]；零删失。**oracle 与机制是同规格的两个独立实现，机制不得 import oracle（AST import 图检查强制）**——共用一份阈值代码会让 claim (i) 退化成「实现 vs 它自己」。**方法学产出**：claim (i) 裁定为与 L1/L2/L3 **正交**的 analytic/specification-conformance，不是第四级；claim (ii) 是普通 L3，只是靶恰好精确。**plan 经三轮红队**（question 已六轮，plan 的 Task 0 此前从未做过）：第 1 轮发现靶 `P_i_grid` 从未被任何 check 钉住（偏 1% 全绿且打印的表不变）；第 2 轮发现「定义」不等于「验证」——把 draw 顺序*声明*一下不能封住角色互换，实测 245,760 个三元组里 109,624 个受影响而角色标签记录让它们**全部逃逸**；第 3 轮钉死 step 口径（含空步 227 vs 只数有效步 89，差 2.55 倍）。**如实记录**：IC-3 的逐位对账在冻结网格上**无法区分**「字面浮点序列」与「先化简再除」两种阈值算法（差异只在 r=5/3，网格外），已写成断言固定；`staging.py` 第五次被绕过。世界文档见 [`docs/zh/worlds/003-moran-selection.md`](../worlds/003-moran-selection.md)。证据：`results/third-world/summary.json` | 判据见预注册（冻结 `aa052dd`，`prereg.sh audit` PASS）；claim (i) 0 失败步、claim (ii) 三 cell 全在域内、零删失，`passed: true`；痛点判定 `not_applicable`（goal 事前声明属方法学收益类） |
...
11. **安全绳仍是运行时前置条件（2026-09-04 部分修复）**：每个 verdict runner 在判定前重跑它全部的上游 runner，而上游又各自重跑自己的上游——递推 f(n) = 1 + Σ_{k<n} f(k)，即 **f(n) = 2^(n-1)**，**指数**。第十九个里程碑实测展开成 17 个子进程（第十五个跑 8 次、第十六 4 次、第十七 2 次），300.66 秒。**已修**：`judgment.py` 用进程树共享的账本去重，每个节点执行一次——同一次判定降到 **6 个子进程、74.89 秒（4.0×）**，`results/nineteenth/summary.json` 逐字节不变、verdict 仍 H1。用文件而非环境变量传递，因为菱形依赖（A→B,C；B,C→D）靠环境变量传不回来；**今天的图恰好没有菱形，但「里程碑排成一条线」正是要拆掉的假设**。**未修的那一半**：安全绳仍然是**运行时前置条件**——「回答新问题」仍以「重跑一批不相关的旧结论」为前置。在 newlife 内部这是对的（里程碑确实一个搭一个），**但对用户的问题就是错的：它们是兄弟不是父子**，50 个问题不该有任何一个把另一个当前置条件。正解是按依赖 DAG 只选受影响的、加内容寻址缓存，**在 newlife 自己这边仍是待办**；**但用户那边已经不走这条路**：`newlife init` 生成的 runner 里，安全绳只有两条——自我复现与环境未变（预注册模板的 F4 明写「不得运行任何其他问题的 runner」）。实测一个问题 32.1 秒。**另一半代价也仍在**：对已判定输入的刻意改动会一层层往下传，第十八个里程碑的修法**四轮才收敛**。
12. **第一次真实用户使用（2026-09-04）挖出一个四个里程碑都没发现的静默缺陷**：`build_composite` 把节点 `interval` 写死成 1.0，按 `dt=0.02` 推进时 process-bigraph 把请求攒着、累够 1.0 才真跑一次——**251 个采样点只有 5 个不同取值，而数字看起来完全合理**（终值 0.9817，解析 0.9933）。**十五/十六/十七/十九个里程碑无一例外用 `run_composite(composite, 1.0, profile)`，`bare_control.py` 三处也写死 1.0，步长与时长恰好相等，于是不匹配从未暴露。** 已修（`7fe921e`）：加 `interval` 参数，且时长不是步长正整数倍时硬失败；默认仍是 1.0，既有判定逐字节不变。**教训不是「这个 bug」，是「四个接第三方的里程碑用的是同一个步长」——换一个参数这件事只有真用户会做。** 同一轮的四个问题（含两份如实保留的 INVALID、一份 H1 真结论）与三条方法学教训，见 [`docs/zh/product/2026-09-04-first-user-questions.md`](../product/2026-09-04-first-user-questions.md)。

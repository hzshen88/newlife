...
| **第十九个里程碑（已判定）：产率是输入还是结果——产品形态的第一次端到端** | **判定（2026-09-04）：H1**，七个单元全过。问题**由积木自己长出来**：`MonodKinetics` 与 `DynamicFBA` 输入签名逐字相同、在同一端口可互换，而 dFBA 内部用的**正是 Monod 摄取律**——**共用同一条摄取律，差别只在摄取之后**。对齐不手填：Monod 的 `km`/`vmax` 运行时从 dFBA 自己的 `kinetic_params` 读出。扫描 `EX_o2_e`：氧受限 → 乙酸 1.216、产率 0.055；氧充足 → 乙酸归零、产率到 0.0863 平台——**教科书式的溢流代谢**。**唯象模型把产率当输入（恒等于声明的 `yield`），机制模型把产率当结果。** 判据重心是负控 Z5：把 `yield` 钉到某一点，它在其余点仍不动——**对齐一个点 ≠ 表示这个自由度**。**边界如实写明**：只证固定形式的 Monod 覆盖不了，不证任何唯象模型都覆盖不了。**撞出一处非确定性**：`PYTHONHASHSEED` 变了 Monod 产率末位就变（第三方路径上有 `set` 迭代）——照第十七个的办法钉住种子并记进产物；补 `flush` 前那行重启提示被 `execve` 吞掉，**重启发生了却没人看得见，那就成静默了**。**产品指标**：221 行（声明 **27** + 判定 **158** + 哈希种子机制 36），**世界侧新增 0 行**——**贵的是判定，不是世界**。世界文档见 [`docs/worlds/019-yield-input-or-outcome.md`](../worlds/019-yield-input-or-outcome.md)。证据：`results/nineteenth/summary.json` | 判据见预注册（冻结 `0e04160`，audit PASS）；goal 判 `achieved` |

...
| **v0.1b staging 可声明化**（第二个预注册，依赖 0.1a） | 声明形状最小化（评审裁定，2026-08-31）：staging schema 置于既有自由 `schedule` 字段（不加字段——`MechanismSpec` dataclass 形状属契约 v1）；profile 校验 DAG 无环；`staging.py` 按拓扑序生成 Composite 编排。判据 = 复现压力测试 harness 手工连线产出的冻结 trace。单 Composite 语义保持开放（风险 3）——0.1b 只证明"多 Composite 编排可从声明推导"，已足以把调度语义从 harness 代码搬进声明 | 冻结 trace 复现；DAG 无环校验负例通过 |

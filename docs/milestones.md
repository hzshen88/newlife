...
| Can the sender-side operator be read off behaviour? | **H0**, and the failing unit is the criteria, not the science. A two-point extrapolation to zero interval recovers whether a third-party `update` returns a delta or a level: the self-test separates a known-delta from a known-level control at full margin, and **3/3 real third-party processes agree with the hand-written `PortBinding.operation`** — `Grow`, `MonodKinetics`, and `DynamicFBA`, the last solving an LP every tick. That slot was previously, in `foreign.py`'s own words, "an assertion, not a fact read off the third party". **But the only blind unit was vacuous**: S5 flipped the *declaration*, and registration F6 forbids the probe from reading declarations — so S5 is implied by S4 and can never go red (proved by exhaustive search in the artifact, not asserted in prose). The criteria were not edited; S3 "every criterion proves it can fail" went red instead. **A real negative control must change the probed object's behaviour, not its declaration.** | `questions/2026-09-07-sender-side-operator-derivable/results/` |
| Do the reproducibility-class criteria have false negatives? | **H0**, and the false negative is real and repairable. "Reproducible" was made executable — rebuild from what the registration records and rerun; a different output or a failed rebuild is the breakage — so every mutation proves it broke something before any criterion is consulted. **`submodule_uninitialised` passes all three criteria and does not run**: the worktree is clean, the commit is reachable on the remote, digest and data checksums both match, and the rebuilt clone has an empty `lib/`. Each criterion misses it independently — `git status --porcelain` is empty for an uninitialised submodule, `git ls-files` records a gitlink rather than a file so its content never enters the digest, and the commit genuinely is on the remote. **Nowhere in a registration can "this repo has a submodule, clone --recursive" be written.** The other three edge cases are `not_a_mutation` and **mean two different things**: `shallow_clone` is genuinely harmless here, while the filter and LFS cases more likely failed to construct over a local `file://` remote — "tested and fine" must not stand in for "could not be built". Bound to git 2.50.1. | `questions/2026-09-07-repro-class-gate-false-negatives/results/` |

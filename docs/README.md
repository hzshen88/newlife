...
| [`zh/`](zh/) | the author | Working documents in Chinese: `design/proposal.md` (the living build plan), `worlds/` (one document per milestone), `product/` (notes from the first real users). Not needed to use the tool. |

The user-facing loop is also written into every research repository that `newlife start`
creates, as `NEXT.md`, and into the skills the AI reads.

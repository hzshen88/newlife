...
#               Ancestry exists only WITHIN one repository: --results
#               pointing outside this repo is a usage error, and a run
#               that examined no output commits reports PARTIAL, never
#               PASS. A summary line must not assert what was not checked.
...
# Vendored into newlife (2026-09-04). Upstream: ~/.codex/skills/preregistering-analysis.
# The layout convention is newlife's: one folder per question, prereg beside its results.
# Override with PREREG_DIR for a different layout.
PREREG_DIR="${PREREG_DIR:-questions}"
...
FAILS=0
WARNS=0
CHRONO_CHECKED=0   # results commits actually examined; 0 means chronology is unproven
...
audit_one() {
    ...
    CHRONO_CHECKED=$((CHRONO_CHECKED + checked))
    if [ "$checked" -eq 0 ]; then
        warn "CHRONOLOGY: NOT established -- no committed outputs examined under:$RESULTS_PATHS"
        note "This audit says nothing about whether any result post-dates the freeze."
    elif [ "$FAILS" -eq "$chrono_fails_before" ]; then
        pass "CHRONOLOGY: every committed output post-dates the freeze ($checked commit(s) examined)"
    fi
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            --results)
                ...
                rp=${2%/}
                # A path in another repository shares no git DAG with the freeze
                # commit, so `git log -- <it>` fails and the loop below examines
                # nothing -- which used to read as "no violations" and PASS.
                # Failing to look is not the same as looking and finding nothing.
                case "$rp" in
                    *..*) die "--results path must not contain '..': $rp" ;;
                    /*)
                        case "$rp/" in
                            "$ROOT"/*) rp=${rp#"$ROOT"/} ;;
                            *) die "--results path is outside this repository:
    $rp
  repository: $ROOT
  CHRONOLOGY is proven from THIS repo's commit ancestry. Outputs living in
  another repository have no ancestry relation to the freeze commit, so their
  order cannot be established here -- and an audit that cannot establish it
  must not report PASS. Commit the outputs in the same repository, or prove
  the ordering by a mechanism that spans both." ;;
                        esac ;;
                esac
                [ -n "$rp" ] || die "--results path resolves to the repository root"
                RESULTS_PATHS="$RESULTS_PATHS $rp"; shift 2 ;;
            -*) die "unknown audit option: $1" ;;
            *)  files="$files $(rel_path "$1")"; shift ;;
        esac
    done

    # A question folder holds its prereg beside its own results/. Derive that,
    # so the common case needs no --results at all. Falling back to the repo-root
    # defaults would examine nothing and report PARTIAL -- correct, but useless.
    if [ -z "$RESULTS_PATHS" ] && [ -n "$files" ]; then
        for f in $files; do
            d=$(dirname "$f")
            [ -d "$ROOT/$d/results" ] && RESULTS_PATHS="$RESULTS_PATHS $d/results"
        done
    fi
    ...
    if [ -z "$files" ]; then
        for f in "$ROOT/$PREREG_DIR"/*/prereg.md "$ROOT/$PREREG_DIR"/*.md; do
    ...
    if [ "$FAILS" -eq 0 ]; then
        if [ "$CHRONO_CHECKED" -gt 0 ]; then
            ...
            # The old wording said "predictions pre-date outputs" even when zero
            # outputs had been examined. A summary line must not assert what the
            # run did not check.
            echo "RESULT: PARTIAL ($WARNS warning(s)) — the registration was never edited after the freeze."

...
def check(rows: dict[str, str], ledger: set[str], waived: str | None,
    ...
    if piloted_env and live_env and piloted_env != live_env:
        problems.append(
            ...
            f"repairing the environment afterwards turns S1 red. A short pilot on a few "
            f"configurations is enough: coverage is the union of every pilot, this check "
            f"reads only the latest."
